...
def QInvertedResidual(input_tensor, hp, expansion, output_channels, stride, kernel_size=3, name_prefix='inv_res'):
    ...
    expand_int = hp.Int(f"{name_prefix}_exp_int", 0, 2)
    ...
    expand_relu_int = hp.Int(f"{name_prefix}_exp_relu_int", 0, 2)
    ...
    x = QActivation(activation=quantized_relu(bits=hp.Choice(f"{name_prefix}_dw_act_bits", [6, 8])), name=f"{name_prefix}_dw_relu")(x)
...
def QDepthwiseSeparableConv2D(input_tensor, hp, output_channels, stride, kernel_size=3, name_prefix='dw_conv'):
    ...
    x = QActivation(activation=quantized_relu(bits=hp.Choice(f"{name_prefix}_act1_bits", [6, 8])), name=f"{name_prefix}_relu1")(x)
...
def QResidual(input_tensor, hp, filters, name_prefix='res'):
    ...
    x = QActivation(activation=quantized_relu(bits=6, integer=0), name=f"{name_prefix}_relu0")(input_tensor)
    x = QConv2D(
    ...
    x = QActivation(activation=quantized_relu(bits=6, integer=0), name=f"{name_prefix}_relu1")(x)
    x = QConv2D(
        filters=filters, kernel_size=3, padding='same', use_bias=False,
...
#------------------------------------------------------------------------------------------------------------------
    #输出模型
    model = keras.Model(inputs=model_input, outputs=x, name='Q_MiDaS_Small_v1')
