...
def O1000():
    ...
    tanhlu_1 = Tanhlu(beta_initializer=initializers.Constant(0.01), name='tanhlu_1')
    x = tanhlu_1(x)

    x = layers.Conv2D(filters=64,kernel_size=3,strides=2,padding='same',use_bias=False,name = 'conv_2')(x)
    x = layers.BatchNormalization(name = 'bn_2')(x)
    tanhlu_2 = Tanhlu(beta_initializer=initializers.Constant(0.01), name='tanhlu_2')
    x = tanhlu_2(x)

    x = layers.Conv2D(filters=128,kernel_size=3,strides=1,padding='same',use_bias=False,name = 'conv_3')(x)
    x = layers.BatchNormalization(name = 'bn_3')(x)
    tanhlu_3 = Tanhlu(beta_initializer=initializers.Constant(0.01), name='tanhlu_3')
    x = tanhlu_3(x)
    ...
    model = keras.Model(inputs = model_input,outputs = x,name = 'O1000_tanhlu')
